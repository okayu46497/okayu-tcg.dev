...
def migrate_official_cards():
    ...
    if not DB_PATH.exists():
        print(
            f"[ERROR] SQLite database {DB_PATH} not found. Cannot migrate official cards."
        )
        return
    ...
    try:
        ...
        chunk_size = 1000
        chunks = [
            db_cards[i : i + chunk_size] for i in range(0, len(db_cards), chunk_size)
        ]
        ...
        for idx, chunk in enumerate(chunks):
            chunk_num = idx + 1
            print(
                f"  [Chunk {chunk_num}/{len(chunks)}] Uploading {len(chunk)} cards..."
            )

            try:
                ins_url = f"{SUPABASE_URL}/rest/v1/official_cards"
                resp = requests.post(
                    ins_url, json=chunk, headers=get_headers(), timeout=30
                )

                if resp.status_code in (200, 201):
                    success_count += len(chunk)
                    print(
                        f"  [Chunk {chunk_num}/{len(chunks)}] SUCCESS (Uploaded {len(chunk)} cards, total success: {success_count})"
                    )
                else:
                    failed_count += len(chunk)
                    print(
                        f"  [Chunk {chunk_num}/{len(chunks)}] ERROR (Status: {resp.status_code}): {resp.text}"
                    )
...
def migrate_cards():
    ...
    if db_cards:
        ...
        ins_resp = requests.post(
            ins_url, json=db_cards, headers=get_headers(), timeout=10
        )
...
def migrate_decks():
    ...
    if db_decks:
        ...
        ins_resp = requests.post(
            ins_url, json=db_decks, headers=get_headers(), timeout=10
        )
        print(f"Insert response: {ins_resp.status_code}")
        if ins_resp.status_code in (200, 201):
